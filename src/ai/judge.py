...
SYSTEM_PROMPT = """あなたはBTCUSD（ビットコイン）専門のプロスキャルパーです。
...
- TPの設定根拠:
  - 次のサポート/レジスタンスレベル、EMAライン、直近の高値/安値を参考に
  - ATRを考慮して現実的な目標を設定
  - TP1/TP2/TP3はそれぞれ異なる根拠に基づくこと（等間隔禁止）
- BUYの場合: SL < Entry < TP1 < TP2 < TP3
- SELLの場合: TP3 < TP2 < TP1 < Entry < SL

必ず有効なJSONのみで返答してください。"""

USER_PROMPT_TEMPLATE = """以下のBTCUSD市場データを分析し、スキャルピングエントリーすべきか判断してください。
